...
class SingleRoleInheritanceTest extends TestCase
{
    ...
    /**
     * Test case-insensitive menu item
     *
     * @return void
     *
     ...
     * @since 6.5.0
     */
    public function testCaseInsensitiveMenuItemAccess()
    {
        $user   = AAM::getUser();
        $object = $user->getObject(AAM_Core_Object_Menu::OBJECT_TYPE);

        // Check if save returns positive result
        $this->assertTrue($object->updateOptionItem('AAM', true)->save());
        ...
        $this->assertTrue($menu->isRestricted('aam'));
        $this->assertTrue($menu->isRestricted('aAm'));
        $this->assertTrue($menu->isRestricted('AAM'));
    }
